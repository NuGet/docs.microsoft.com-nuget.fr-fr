...
- [NuGet dans Visual Studio](#nuget-in-visual-studio)
- [Ligne de commande NuGet](#nuget-command-line)
- [Console du Gestionnaire de package NuGet](#nuget-package-manager-console)
- [Création et publication de packages](#creating-and-publishing-packages)
- [Utilisation des packages](#working-with-packages)
- [Gestion des packages sur nuget.org](#managing-packages-on-nugetorg)
- [nuget.org inaccessible](#nugetorg-not-accessible)

## <a name="getting-started"></a>Bien démarrer

**Quels sont les éléments nécessaires pour exécuter NuGet ?**

Toutes les informations relatives à l’interface utilisateur et aux outils en ligne de commande sont disponibles dans le [guide d’installation](../guides/install-nuget.md).
...
Oui, NuGet fonctionne directement à partir de la ligne de commande. Consultez le [guide d’installation](../guides/install-nuget.md) et les [informations de référence sur l’interface de ligne de commande](../tools/nuget-exe-CLI-Reference.md).
...
Consultez le [guide d’installation](../guides/install-nuget.md).
...
Définissez le paramètre [`repositoryPath`](../Schema/nuget-config-file.md#config-section) dans `Nuget.Config` en utilisant `nuget config -set repositoryPath=<path>`.
...
- Supprimez le `%appdata%\.nuget\NuGet.Config` et laissez NuGet le recréer.
...
Oui. Vous pouvez réserver des ID pour les packages sur [nuget.org](https://www.nuget.org/) en demandant un préfixe d’ID de package pour votre compte. Pour demander un préfixe d’ID de package, envoyez un e-mail à account (at) nuget.org avec le nom complet du propriétaire du package et le préfixe d’ID de package demandé.  
