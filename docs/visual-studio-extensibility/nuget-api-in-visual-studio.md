...
Au fil des années, NuGet a ajouté de nombreux services qui résident dans l' `NuGet.VisualStudio` espace de noms de l' `NuGet.VisualStudio.dll` assembly :

Depuis NuGet 3.3 +, NuGet exporte les éléments suivants :

- [`IRegistryKey`](#iregistrykey-interface): Méthode pour récupérer une valeur d’une sous-clé de registre. (3.3 +)
- [`IVsCredentialProvider`](#ivscredentialprovider-interface) Contient des méthodes pour obtenir des informations d’identification pour les opérations NuGet. (4.0 +)
- [`IVsFrameworkCompatibility`](#ivsframeworkcompatibility-interface) Contient des méthodes permettant de découvrir les infrastructures et la compatibilité entre les infrastructures. (4.0 +)
- [`IVsFrameworkCompatibility2`](#ivsframeworkcompatibility2-interface) Contient des méthodes permettant de découvrir les infrastructures et la compatibilité entre les infrastructures. (4.0 +)
- [`IVsFrameworkCompatibility3`](#ivsframeworkcompatibility3-interface) Contient des méthodes permettant de découvrir les infrastructures et la compatibilité entre les infrastructures. (5.8 +)
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) Interface pour traiter la conversion entre des chaînes et [FrameworkName](/dotnet/api/system.runtime.versioning.frameworkname) (4.0 +)
- [`IVsFrameworkParser2`](#ivsframeworkparser2-interface) Interface pour analyser des chaînes de .NET Framework. Consultez [NuGet-IVsFrameworkParser](https://aka.ms/NuGet-IVsFrameworkParser). (5.8 +)
- [`IVsGlobalPackagesInitScriptExecutor`](#ivsglobalpackagesinitscriptexecutor-interface) Exécuter des scripts PowerShell à partir d’un ou de plusieurs packages dans une solution (4.0 +)
- [`IVsNuGetFramework`](#ivsnugetframework-interface) Type qui représente les composants d’un moniker de Framework cible .NET. (5.8 +)
- [`IVsPackageInstaller`](#ivspackageinstaller-interface): Méthodes permettant d’installer des packages NuGet dans des projets. (3.3 +)
- [«IVsPackageInstaller2](#ivspackageinstaller2-interface) Contient une méthode pour installer la dernière version d’un package unique dans un projet au sein de la solution actuelle.
- [`IVsPackageInstallerEvents`](#ivspackageinstallerevents-interface): Événements pour l’installation/la désinstallation du package. (3.3 +)
- [`IVsPackageInstallerProjectEvents`](#ivspackageinstallerprojectevents-interface): Événements batch pour l’installation/la désinstallation du package. (3.3 +)
- [`IVsPackageInstallerServices`](#ivspackageinstallerservices-interface): Méthodes pour récupérer les packages installés dans la solution actuelle et pour vérifier si un package donné est installé dans un projet. (3.3 +)
- [`IVsPackageManagerProvider`](#ivspackagemanagerprovider-interface): Méthodes pour fournir d’autres suggestions du gestionnaire de package pour un package NuGet. (3.3 +)
- [`IVsPackageMetadata`](#ivspackagemetadata-interface): Méthodes pour récupérer des informations sur un package installé. (3.3 +)
- [`IVsPackageProjectMetadata`](#ivspackageprojectmetadata-interface): Méthodes pour récupérer des informations sur un projet dans lequel les actions NuGet sont en cours d’exécution. (3.3 +)
- [`IVsPackageRestorer`](#ivspackagerestorer-interface): Méthodes permettant de restaurer les packages installés dans un projet. (3.3 +)
- [`IVsPackageSourceProvider`](#ivspackagesourceprovider-interface): Méthodes pour récupérer une liste de sources de package NuGet. (3.3 +)
- [`IVsPackageUninstaller`](#ivspackageuninstaller-interface): Méthodes permettant de désinstaller les packages NuGet des projets. (3.3 +)
- [`IVsPathContext`](#ivspathcontext-interface) Informations de chemin d’accès NuGet spécifiques au contexte actuel (par exemple, le contexte de projet). (4.0 +)
- [`IVsPathContext2`](#ivspathcontext2-interface) Informations de chemin d’accès NuGet spécifiques au contexte actuel (par exemple, le contexte de projet). (5.0 +)
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) Fabrique pour initialiser les instances [IVsPathContext](#ivspathcontext-interface) . (4.0 +)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) Fabrique pour initialiser les instances [IVsPathContext2](#ivspathcontext2-interface) . (5.0 +)
- [`IVsProjectJsonToPackageReferenceMigrateResult`](#ivsprojectjsontopackagereferencemigrateresult-interface)  Contient le résultat de l’opération de migration sur un project.jshérité sur le projet (4.3 +)
- [`IVsProjectJsonToPackageReferenceMigrator`](#ivsprojectjsontopackagereferencemigrator-interface) Contient des méthodes pour migrer un project.jssur un projet hérité basé sur PackageReference. (4.3 +)
- [`IVsSemanticVersionComparer`](#ivssemanticversioncomparer-interface) Interface pour comparer deux chaînes de version opaque en les traitant comme sémantique NuGet (4.0 +)
- [`IVsTemplateWizard`](#ivstemplatewizard-interface): Conçu pour les modèles de projet/élément pour inclure des packages préinstallés ; Cette interface n’est *pas* destinée à être appelée à partir du code et n’a pas de méthode publique. (3.3 +)

## <a name="using-nuget-services"></a>Utilisation des services NuGet

1. Installez le [`NuGet.VisualStudio`](https://www.nuget.org/packages/NuGet.VisualStudio) package dans votre projet, qui contient l' `NuGet.VisualStudio.dll` assembly.
...
1. Pour utiliser un service, importez-le via [l’attribut d’importation MEF](/dotnet/framework/mef/index#imports-and-exports-with-attributes) ou via le [service IComponentModel](/dotnet/api/microsoft.visualstudio.componentmodelhost.icomponentmodel).
