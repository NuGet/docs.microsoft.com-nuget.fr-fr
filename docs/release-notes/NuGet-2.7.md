...
[Notes de publication de NuGet 2.6.1 pour WebMatrix](../release-notes/nuget-2.6.1-for-webmatrix.md)  |  [Notes de publication de NuGet 2.7.1](../release-notes/nuget-2.7.1.md)
...
2. [Adam Ralph](http://www.codeplex.com/site/users/view/adamralph) ( [@adamralph](https://twitter.com/adamralph) )
    - [#1956](http://nuget.codeplex.com/workitem/1956) -ajouter l’attribut developmentDependency à `packages.config` et l’utiliser dans la commande Pack pour inclure uniquement des packages Runtime
3. [Rafael Nicoletti](http://www.codeplex.com/site/users/view/tkrafael) ( [@tkrafael](https://twitter.com/tkrafael) )
    - Évitez la clé des propriétés dupliquées dans nuget.exe commande Pack.
...
10. [Justin-chère](http://www.codeplex.com/site/users/view/zippy1981) ( [@zippy1981](https://twitter.com/zippy1981) )
     - [#3307](http://nuget.codeplex.com/workitem/3307) -Fix `NuGet.targets` ne passe pas la valeur $ (Platform) à nuget.exe lors de la restauration des packages.
11. [Brian Federici](http://www.codeplex.com/site/users/view/benerdin)
     - [#3294](http://nuget.codeplex.com/workitem/3294) -corriger le bogue dans nuget.exe commande de package qui permet d’ajouter des fichiers portant le même nom mais avec une casse différente, provoquant éventuellement l’exception « élément déjà existant ».
...
NuGet 2,7 introduit une nouvelle approche de la restauration des packages, et est également un obstacle majeur : le consentement de la restauration des packages est maintenant activé par défaut. La combinaison de la nouvelle approche et du consentement implicite simplifie considérablement les scénarios de restauration des packages.
...
* Visual Studio 2012
* Visual Studio 2010
* Utilitaire de Command-Line nuget.exe
...
#### <a name="simplified-package-restore-from-the-command-line"></a>Restauration simplifiée des packages à partir de la Command-Line
...
* AppHarbor : ils travaillent pour vous permettre de pousser vos référentiel vers AppHarbor et de les `nuget.exe restore` appeler avant la génération de votre solution.

Avec chacun des partenaires ci-dessus, ils utilisent leur propre copie de nuget.exe et vous n’avez pas besoin de transporter des nuget.exe dans votre solution.

#### <a name="known-issues"></a>Problèmes connus

Il y a eu deux problèmes connus avec nuget.exe restaurer avec la version 2,7 initiale, mais ils ont été corrigés le 9/6/2013 avec une mise à jour du [package NuGet. CommandLine](http://www.nuget.org/packages/NuGet.CommandLine/).  Cette mise à jour est également disponible sur la [page de téléchargement de NuGet 2,7](https://nuget.codeplex.com/releases/view/107605) sur CodePlex.  `nuget.exe update -self`L’exécution de est mise à jour vers la dernière version.
...
1. La source d’envoi par défaut nuget.exe

Chacune d’elles peut désormais être configurée dans un fichier situé à l’emplacement `%ProgramData%\NuGet\NuGetDefaults.Config` . Si ce fichier de configuration spécifie des sources de package, la source du package nuget.org par défaut n’est pas inscrite automatiquement, et celles dans `NuGetDefaults.Config` sont inscrites à la place.

Bien qu’il ne soit pas nécessaire d’utiliser cette fonctionnalité, nous pensons que les entreprises déploient des `NuGetDefaults.Config` fichiers à l’aide de stratégie de groupe.

*Notez que cette fonctionnalité ne provoque jamais la suppression d’une source de package des paramètres NuGet d’un développeur. Cela signifie que si le développeur a déjà utilisé NuGet et qu’il a par conséquent enregistré la source du package nuget.org, il ne sera pas supprimé après la création d’un `NuGetDefaults.Config` fichier.*
