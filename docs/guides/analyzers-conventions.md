...
- **framework_name** et **la version**: la surface *API optionnelle* du cadre .NET que les DLL contenus doivent fonctionner. `dotnet` est actuellement la seule valeur valide, car Roslyn est le seul hôte qui peut exécuter des analyseurs. Si aucune cible n’est spécifiée, les DLL sont censées s’appliquer à *toutes* les cibles.
