...
-------- | ------ | ------- | -------- | -----
LOWER_ID | URL    | string  | yes      | ID de package, en minuscules

La `LOWER_ID` valeur est l’ID de package souhaité en minuscules à l’aide des règles implémentées par. Méthode du réseau [`System.String.ToLowerInvariant()`](/dotnet/api/system.string.tolowerinvariant?view=netstandard-2.0#System_String_ToLowerInvariant) .
...
----- | ---------------- | -------- | -----
count | entier          | yes      | Nombre de pages d’inscription dans l’index
items | tableau d’objets | yes      | Tableau des pages d’inscription
...
Nom   | Type             | Obligatoire | Notes
------ | ---------------- | -------- | -----
@id    | string           | yes      | URL de la page d’inscription
count  | entier          | yes      | Nombre d’inscriptions laissées dans la page
items  | tableau d’objets | non       | Le tableau de l’inscription quitte et leurs métadonnées associées
lower  | string           | yes      | La version SemVer 2.0.0 la plus basse dans la page (inclusive)
parent | string           | non       | URL de l’index d’inscription
upper  | string           | yes      | Version SemVer 2.0.0 la plus élevée dans la page (inclusive)
...
Nom           | Type   | Obligatoire | Notes
-------------- | ------ | -------- | -----
@id            | string | yes      | URL de la feuille d’inscription
catalogEntry   | object | yes      | Entrée de catalogue contenant les métadonnées du package
packageContent | string | yes      | URL du contenu du package (. nupkg)
...
Nom                     | Type                       | Obligatoire | Notes
------------------------ | -------------------------- | -------- | -----
@id                      | string                     | yes      | URL du document utilisé pour produire cet objet
...
title                    | string                     | non       | 
version                  | string                     | yes      | Chaîne de version complète après la normalisation
...
Nom         | Type   | Obligatoire | Notes
------------ | ------ | -------- | -----
id           | string | yes      | ID de la dépendance du package
...
Nom             | Type             | Obligatoire | Notes
---------------- | ---------------- | -------- | -----
principales          | tableau de chaînes | yes      | Raisons pour lesquelles le package a été déconseillé
...
Nom         | Type   | Obligatoire | Notes
------------ | ------ | -------- | -----
id           | string | yes      | ID de l’autre package
...
@id    | string           | yes      | URL de la page d’inscription
count  | entier          | yes      | Nombre d’inscriptions laissées dans la page
items  | tableau d’objets | yes      | Le tableau de l’inscription quitte et leurs métadonnées associées
lower  | string           | yes      | La version SemVer 2.0.0 la plus basse dans la page (inclusive)
parent | string           | yes      | URL de l’index d’inscription
upper  | string           | yes      | Version SemVer 2.0.0 la plus élevée dans la page (inclusive)
