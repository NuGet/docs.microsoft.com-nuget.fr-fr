...
Cette API est utilisée par le client NuGet dans Visual Studio, NuGet. exe et l’interface CLI .NET pour effectuer des opérations NuGet comme [`dotnet restore`](/dotnet/core/tools/dotnet-restore?tabs=netcore2x), effectuer une recherche dans l’interface utilisateur de Visual Studio et [`nuget.exe push`](../reference/cli-reference/cli-ref-push.md).
...
> [!Note]
> Lorsqu’une source n’implémente pas `SearchAutocompleteService` le comportement de la saisie semi-automatique doit être désactivé correctement. Lorsque `ReportAbuseUriTemplate` n’est pas implémentée, le client NuGet officiel revient à l’URL d’abus des rapports de NuGet. org (suivie par [NuGet/4924](https://github.com/NuGet/Home/issues/4924)). D’autres clients peuvent choisir de ne pas afficher une URL d’abus de rapport à l’utilisateur.
...
Troisièmement, nous ne documentons pas les ressources qui sont étroitement couplées avec le protocole v2, qui lui-même est intentionnellement non documenté. La ressource `LegacyGallery` appartient à cette catégorie. Cette ressource permet à un index de service v3 de pointer vers une URL source v2 correspondante. Cette ressource prend en charge les `nuget.exe list`.
...
HEAD   | Récupère les en-têtes de réponse pour la demande de `GET` correspondante.
...
Toute demande de `GET` adressée à un point de terminaison d’API peut retourner une redirection HTTP (301 ou 302). Les clients doivent gérer ces redirections de manière appropriée en observant l’en-tête `Location` et en émettant une `GET`suivante. La documentation relative à des points de terminaison spécifiques n’appellera pas explicitement l’endroit où les redirections peuvent être utilisées.
...
Name                     | Description
------------------------ | -----------
X-NuGet-ApiKey           | Requis pour l’envoi et la suppression, consultez [`PackagePublish` ressource](package-publish-resource.md)
...
La `X-NuGet-Session-Id` a une valeur unique pour toutes les opérations liées à une restauration unique dans `PackageReference`. Pour d’autres scénarios tels que la saisie semi-automatique et la restauration de `packages.config`, il peut y avoir plusieurs ID de session différents en raison de la façon dont le code est pondéré.

## <a name="authentication"></a>Authentication

L’authentification est laissée à l’implémentation de la source du package à définir. Pour nuget.org, seule la ressource `PackagePublish` nécessite une authentification via un en-tête de clé API spécial. Pour plus d’informations, consultez [`PackagePublish` ressource](package-publish-resource.md) .
