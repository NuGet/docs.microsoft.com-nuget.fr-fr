...
- `defaultPushSource`: spécifie la cible par défaut pour les `nuget push` opérations, en substituant la valeur par défaut intégrée de NuGet.org. Les administrateurs peuvent déployer ce paramètre pour éviter la publication de packages internes sur le nuget.org public par accident, car les développeurs doivent spécifiquement utiliser `nuget push -Source` pour publier sur NuGet.org.
